...
function createDoor ()
  local wClose,h, wOpen = 20, 70, 50 
  
  local posX, posY = 0, (screenHeight - h)
  
  return {
    draw = function ()
      if isGameOver == false then
        love.graphics.setColor(255,255,0)
        love.graphics.rectangle("fill",posX,posY, wClose,h)
        if isDoorOpen == true then
          love.graphics.rectangle("fill",posX + wClose + 5,posY, wOpen,h)
        end
      end
